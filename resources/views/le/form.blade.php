@php
    ...
    $title = 'create'; $method = 'post'; $action = 'le.store';
    if ($segment !== 'create' ) { $title = 'edit'; $method = 'put'; $action = ['le.update', $data->{$data->getKeyName()}]; }
@endphp
...
@section('subcontent')
    ...
    {{ Form::model($data, ['route' => $action, 'method' => $method, 'class' => 'form-horizontal form-material form-data', 'autocomplete' => 'off']) }}
        ...
        @include('...layouts.partials.admin.button_form', [ 'backLink' => 'le.index'])
    {{ Form::close() }}
